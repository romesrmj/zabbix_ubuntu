...
# Configurar o plugin Zabbix com as informações do banco de dados Zabbix
# Certifique-se de ajustar as variáveis de acordo com a sua instalação
...
# Ativar o painel do Zabbix no Grafana
loading_message "Ativando painel Zabbix no Grafana" 3
# Adicionar o painel padrão do Zabbix, você pode adicionar mais painéis conforme necessário
curl -X POST -H "Content-Type: application/json" \
    -d '{
          "dashboard": {
    ...
    http://admin:admin@localhost:3000/api/dashboards/db || error_message "Erro ao configurar o painel do Zabbix no Grafana"
